...
try:
    ...
    def _v41_cfg_init_text_only(self, *args, **kwargs):
        ...
        # SM120 kernel contract: prefill topk must stay at the SWA window
        # (128) because the DSv4 dual-lane prefill kernel rejects topk>128
        # when the compressed lane is present. Attention-path readers use
        # dsv41_attention_vmnt (=0). Image PREPROCESSING still needs a real
        # budget, so vision_max_n_token keeps a workable value (896).
...
# DSV41_PROBE: log flashinfer DSV4 sparse kernel call signatures (debug only).
import os as _probe_os
if _probe_os.environ.get("DSV41_PROBE") == "1":
    try:
        import probe_fi
    except Exception as _e:
        import sys as _s
        print(f"probe failed: {_e!r}", file=_s.stderr)
